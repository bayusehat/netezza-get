...
# Run SQL Netezza
sql_str = """SELECT nper, nd, is_paid FROM telkombda..CHURN_SYMPTOM_GROUP_BILLPER WHERE nper = '202212' AND is_paid = 1 AND to_char(PAYMENT_DATE, 'YYYYMMDD') IN ('{}', '{}')""".format(ystrday,now)
curs = conn.cursor()
...
trunc_ctc = cursor.execute("""truncate table public.ctc_bilper_update_paid""")
connection.commit()
for a in result:
    insert_ctc = """insert into public.ctc_bilper_update_paid (nper, nd, is_paid) values ('{}','{}','{}')""".format(
        a[0], a[1], a[2])
...
#update current
update_query = """UPDATE
  public.ctc_dapros_bilper a
SET
  is_paid = b.is_paid,
  updated_at = now()
FROM
  public.ctc_bilper_update_paid b
WHERE
  1 = 1
  AND a.nd = b.nd
  AND b.is_paid = 1
